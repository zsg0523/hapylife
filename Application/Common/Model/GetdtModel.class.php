		...
		if($type==1){
			if($endtime){
				$count=$model
				    ...
				    ->where(array('dttype'=>array('in','6'),'date'=>array(array('egt',$starttime),array('elt',$endtime))))
				...
				$count=$model
		            ...
		            ->join('__USER__ iu ON ig.iuid=iu.iuid')
		            ->field('igid,pointno,ig.hu_nickname,ig.hu_username,ig.iu_bank,ig.iu_bankbranch,ig.iu_bankbranch,ig.iu_bankaccount,ig.iu_bankprovince,ig.iu_bankcity,iu.teamCode,ig.iuid,getpoint,feepoint,realpoint,ig.date,opename,handletime,content,ig.status')
				    ->where(array('dttype'=>array('in','6'),'date'=>array(array('egt',$starttime))))
					->where(array('ig.hu_nickname|pointNo|ig.hu_username|iu.BankName|iu.SubName|iu.BankAccount|teamcode'=>array('like','%'.$word.'%'),'status'=>array('in',$status)))
					->count();
			}
		}else{
			if($starttime){
				if($endtime){
					$count=$model
			            ...
			            ->join('__USER__ iu ON ig.iuid=iu.iuid')
			            ->field('igid,pointno,ig.hu_nickname,ig.hu_username,ig.iu_bank,ig.iu_bankbranch,ig.iu_bankbranch,ig.iu_bankaccount,ig.iu_bankprovince,ig.iu_bankcity,iu.teamCode,ig.iuid,getpoint,feepoint,realpoint,ig.date,opename,handletime,content,ig.status')
					    ->where(array('dttype'=>array('in','6'),'handletime'=>array(array('egt',$starttime),array('elt',$endtime))))
						->where(array('ig.hu_nickname|pointNo|ig.hu_username|iu.BankName|iu.SubName|iu.BankAccount|teamcode'=>array('like','%'.$word.'%'),'status'=>array('in',$status)))
						->count();
				}else{
					$count=$model
			            ...
			            ->join('__USER__ iu ON ig.iuid=iu.iuid')
			            ->field('igid,pointno,ig.hu_nickname,ig.hu_username,ig.iu_bank,ig.iu_bankbranch,ig.iu_bankbranch,ig.iu_bankaccount,ig.iu_bankprovince,ig.iu_bankcity,iu.teamCode,ig.iuid,getpoint,feepoint,realpoint,ig.date,opename,handletime,content,ig.status')
					    ->where(array('dttype'=>array('in','6'),'handletime'=>array(array('egt',$starttime))))
						->where(array('ig.hu_nickname|pointNo|ig.hu_username|iu.BankName|iu.SubName|iu.BankAccount|teamcode'=>array('like','%'.$word.'%'),'status'=>array('in',$status)))
						->count();
				}
			}else{
				if($endtime){
					$count=$model
			            ...
			            ->join('__USER__ iu ON ig.iuid=iu.iuid')
			            ->field('igid,pointno,ig.hu_nickname,ig.hu_username,ig.iu_bank,ig.iu_bankbranch,ig.iu_bankbranch,ig.iu_bankaccount,ig.iu_bankprovince,ig.iu_bankcity,iu.teamCode,ig.iuid,getpoint,feepoint,realpoint,ig.date,opename,handletime,content,ig.status')
					    ->where(array('dttype'=>array('in','6'),'handletime'=>array(array('elt',$endtime))))
						->where(array('ig.hu_nickname|pointNo|ig.hu_username|iu.BankName|iu.SubName|iu.BankAccount|teamcode'=>array('like','%'.$word.'%'),'status'=>array('in',$status)))
						->count();
				}else{
					$count=$model
        	...
        	foreach ($value as $k => $v) {
        		...
        		switch ($v['dttype']) {
        			case '1':
        				...
        				break;
	        		case '2':
	        			$arr[$key]['realpoint2'] = bcadd($arr[$key]['realpoint1'],$v['getdt'],4);
	        			$arr[$key]['dttype2'] = $v['dttype'];
	        			break;
	        		case '3':
	        			$arr[$key]['realpoint3'] = bcadd($arr[$key]['realpoint3'],$v['getdt'],4);
